$(document).ready(function () {
    ...
    function GetProjects() {
        $.ajax({
            ...
            success: function (resp) {
                ...
                let def = `<option value="0">Select project</option>`;
                $("select[name='ProjectId']").append(def);
                $.each(resp, function (key, project) {
                    let opt = `<option value="${project.Id}">${project.Name}</option>`;
                    $("select[name='ProjectId']").append(opt);
                });
            }
        })
    }
    function GetTeams() {
        $.ajax({
            ...
            success: function (resp) {
                ...
                let def = `<option value="0">Select team</option>`;
                $("select[name='TeamId']").append(def);
                $.each(resp, function (key, team) {
                    let opt = `<option value="${team.Id}">${team.Name}</option>`;
                    $("select[name='TeamId']").append(opt);
                });
            }
        })
    }
    ...
    $("#task-form").submit(function (e) {
        ...
        if ($(this).data("type") == "create") {
            ...
            var project = $("#task-form").find("select[name='ProjectId']").val();
